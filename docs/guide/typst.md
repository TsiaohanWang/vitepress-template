...
- `@preview/*` 包（`cetz` / `alchemist` / `lilaq` / `fletcher` / `tiaoma` 等）首次使用时自动下载至 `~/.cache/typst/packages`；CI 需允许该网络访问，或预先缓存（本仓库 CI 已配置该目录缓存）
...
实现见 `.vitepress/typst.ts`：markdown-it fence 渲染器覆写 + 带缓存的编译封装。站点示例页 [Typst 图表示例](/examples/typst-diagrams) 含 `cetz`（官方 gallery `karls-picture`）/ `alchemist` / `lilaq` / `fletcher` / `tiaoma` 五例。

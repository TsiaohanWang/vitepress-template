...
- **提交活动日历**：全局 `<ActivityCalendar />` 组件构建期读取仓库 git 历史，渲染 GitHub 风格提交热力图（site/page 双粒度）；非 home 内容页底部自动展示本页日历，全站开关为 `themeConfig.autoPageActivityCalendar`
...
│  │  │  └─ AutoActivityCalendar.vue # 内容页底部自动挂载本页日历的包装组件
...
| 提交活动日历 | 非 home 内容页底部自动渲染本页 git 提交热力图（`<ActivityCalendar />`，可开关） |

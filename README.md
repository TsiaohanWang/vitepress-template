...
- **提交活动日历**：全局 `<ActivityCalendar />` 组件构建期读取仓库 git 历史，渲染 GitHub 风格提交热力图（site/page 双粒度）；可选开启内容页底部自动挂载本页日历（`themeConfig.autoPageActivityCalendar`，默认关闭）
...
| 提交活动日历 | `<ActivityCalendar />` git 提交热力图；可选开启内容页底部自动挂载（默认关闭） |
